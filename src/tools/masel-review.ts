 ...
 */
export async function maselReview(options: ReviewOptions): Promise<ReviewReport> {
 ...
 * Check output format
 */
function checkOutputFormat(output: string): number {
 ...
 */
function evaluateEfficiency(results: SubtaskResult[]): { score: number; comments: string[] } {
  const comments: string[] = [];
 ...
 */
async function evaluateReadability(
  results: SubtaskResult[]
): Promise<{ score: number; comments: string[] }> {
  const comments: string[] = [];
  let totalScore = 0;
 ...
 */
function makeDecision(overall_score: number, issues: Issue[]): "APPROVE" | "NEEDS_REVISION" | "REJECT" {
 ...
 * Generate recommendations
 */
function generateRecommendations(issues: Issue[], dimensions: DimensionScore[]): string[] {
 ...
 * Save review report
 */
async function saveReviewReport(report: ReviewReport): Promise<void> {
 ...
 */
async function recordIssuesForLearning(report: ReviewReport): Promise<void> {
