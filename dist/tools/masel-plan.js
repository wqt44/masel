...
async function maselPlan(options) {
    ...
    const estimated_total_time = subtasks.reduce((sum, st) => sum + st.estimated_time, 0);
 ...
 */
async function performBrainstorming(task, workflow_type) {
    // Search for similar historical tasks
    const similar_tasks = await (0, openclaw_api_js_1.memory_search)({
    ...
    switch (workflow_type) {
        case "coding":
            approaches.push("Direct implementation: Write code immediately", "TDD approach: Write tests first, then implementation", "Prototype first: Build quick prototype, then refine", "Modular design: Break into reusable components");
            break;
        case "research":
            approaches.push("Broad search: Gather comprehensive information", "Deep dive: Focus on specific aspects", "Comparative analysis: Compare multiple sources", "Trend analysis: Identify patterns over time");
            break;
        case "complex":
            approaches.push("Waterfall: Sequential phases", "Iterative: Multiple iterations", "Parallel: Multiple workstreams", "Agile: Flexible adaptation");
            break;
        default: // simple
            approaches.push("Direct execution: Single step", "Two-phase: Quick check then execute");
    }
 ...
 */
function selectBestApproach(task, approaches, similar_tasks) {
    // Check task complexity indicators
    const complexity_indicators = [
        "and", "then", "after", "before", "multiple",
        "complex", "integrate", "system", "architecture"
    ];
    const complexity_score = complexity_indicators.reduce((score, indicator) => score + (task.toLowerCase().includes(indicator) ? 1 : 0), 0);
 ...
 */
async function refineSpec(task, brainstorm, workflow_type) {
 ...
 * Generate acceptance criteria
 */
function generateAcceptanceCriteria(task, workflow_type) {
    const criteria = [];
    // Generic criteria
 ...
 * Identify constraints
 */
function identifyConstraints(task) {
    const constraints = [];
    // Time constraints
 ...
 * Define boundary conditions
 */
function defineBoundaryConditions(task) {
    return [
 ...
 */
async function createSubtasks(spec, workflow_type) {
    const subtasks = [];
    switch (workflow_type) {
        case "coding":
            subtasks.push({
...
exports.default = maselPlan;
//# sourceMappingURL=masel-plan.js.map